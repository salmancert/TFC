...
def default_output_path(statement_path: str) -> str:
    stem, _ = os.path.splitext(statement_path)
    return f"{stem}_reconciled.xlsx"
...
class ReconcilerApp:
    ...
    def _build(self) -> None:
        ...
        ttk.Label(source, text="Bank statement sheet:").grid(row=1, column=0, sticky="w", **padding)
        ...
        ttk.Label(source, text="Ledger sheet:").grid(row=4, column=0, sticky="w", **padding)
        ...
        ttk.Label(source, text="Save coloured copy to:").grid(
    ...
    def _browse_statement(self) -> None:
        ...
        path = filedialog.askopenfilename(title="Choose the statement", filetypes=SPREADSHEET_TYPES)
        if not path:
            return
        self.statement_path.set(path)
        self.output_path.set(default_output_path(path))
        if not self.separate_ledger_file.get():
            self.ledger_path.set(path)
            self._load_sheets(path, which="both")
        else:
            self._load_sheets(path, which="statement")
